...
- Every envelope includes `sequence`; it is non-null for telemetry and `null` for
  non-telemetry events.
- Delivered out-of-order telemetry is broadcast and persisted like any other delivered
  sample; sequence gaps, duplicates, and ordering are also counted in metrics.
- The simulator creates deterministic event IDs from the run ID, event type, and
  ordinal/vehicle sequence. The API preserves those IDs during publication and
  persistence.
