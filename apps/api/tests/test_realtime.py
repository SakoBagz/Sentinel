...
def _auth_qs(client: TestClient) -> str:
...
def _run_id(client: TestClient) -> str:
    ...
    client.post(
        ...
        json={"vehicle_id": vehicle["id"], "sequence": 0, "latitude": 34.151, "longitude": -118.24, "altitude_m": 100},
    )
...
def test_websocket_subscribe_and_ping(client) -> None:
    with client.websocket_connect(f"/ws/runs/{_run_id(client)}?{_auth_qs(client)}") as websocket:
...
def test_websocket_rejects_unknown_run(client) -> None:
    with client.websocket_connect(f"/ws/runs/{uuid4()}?{_auth_qs(client)}") as websocket:
