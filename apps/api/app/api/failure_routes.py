...
async def create_failure(
    run_id: UUID, payload: FailureCreate, session: AsyncSession = Depends(get_db_session)
) -> FailureRead:
    try:
        return FailureRead.model_validate(await failure_service.inject_failure(session, run_id, payload))
...
async def clear_failure(
    run_id: UUID, failure_id: UUID, session: AsyncSession = Depends(get_db_session)
) -> FailureRead:
    try:
        return FailureRead.model_validate(await failure_service.clear_failure(session, run_id, failure_id))
