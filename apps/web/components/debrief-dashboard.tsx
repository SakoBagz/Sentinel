...
export function DebriefDashboard({ runId }: { runId: string }) {
  ...
  const runAnalyst = async (action: () => Promise<AnalystResponse>) => {
    ...
    catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "Mission Analyst unavailable"); }
  ...
  return (
    <main className="main">
      ...
      {error && <div className="notice error" role="alert"><strong>Mission Analyst notice.</strong> {error}</div>}
