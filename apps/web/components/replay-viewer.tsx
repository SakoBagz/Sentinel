...
export function ReplayViewer({ runId }: { runId: string }) {
  ...
  useEffect(() => { Promise.all([getReplay(runId), getEvents(runId)]).then(([telemetry, history]) => { setSamples(telemetry); setEvents(history); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Unable to load replay")); }, [runId]);
  ...
  return <main className="main"><div className="eyebrow">Replay / {runId}</div><h1>Historical mission</h1><div className="card"><div className="replay-toolbar"><button className="button primary" onClick={() => setPlaying((value) => !value)}>{playing ? "Pause" : "Play"}</button><label className="speed">Time <input type="range" min={0} max={Math.max(1, end)} value={time} onChange={(event) => { setPlaying(false); setTime(Number(event.target.value)); }} /></label><span>{time} ms / {end} ms</span></div><div className="replay-map"><div className="eyebrow">Persisted vehicle state · {current.length} vehicles</div><div className="replay-vehicles">{current.map((sample) => <div className="replay-vehicle" key={sample.vehicle_id}><strong>{sample.vehicle_id.slice(0, 8)}</strong><span>{sample.latitude?.toFixed(4)}, {sample.longitude?.toFixed(4)} · {sample.mission_state}</span></div>)}</div></div></div><div className="grid replay-lower"><div className="card"><div className="eyebrow">Event timeline</div><div className="events">{events.map((event) => <Link href={`/runs/${runId}/replay?time=${event.sim_time_ms}`} className={`event ${event.severity.toLowerCase()}`} key={event.id}><strong>{event.event_type}</strong><span>{event.sim_time_ms} ms · {event.vehicle_id?.slice(0, 8) ?? "SYSTEM"}</span></Link>)}</div></div><div className="card"><div className="eyebrow">Replay contract</div><p className="card-copy">This view reads durable telemetry and events. It never reruns the simulator.</p><div className="metric"><span>Samples loaded</span><strong>{samples.length}</strong></div><div className="metric"><span>Events loaded</span><strong>{events.length}</strong></div></div></div></main>;
}
